...
### Data visualizations
First, we'll take the New Orleans Vacation Rental data and calculate the number of units by `Type`.
...
```{r bar}
ggplot(no_stats) +
    geom_col(aes(x=Type,y=num_units))
```

We see that the chart produced doesn't look that great. `ggplot` allows us a lot of flexibility. Let's rotate the chart and add a `theme` to make things look a little better.

```{r bar2}
ggplot(no_stats) +
    geom_col(aes(y=Type,x=num_units)) +
    theme_bw()
```

Now, we'll show how we can create an even better looking visualization by creating a custom theme, labeling axes and adding some color. Note that we'll filter the data a little before we do all that.

```{r bar3}
no_stats_short <- no_stats |> filter(num_units >= 100)
...
ggplot(no_stats_short) +
    geom_col(aes(y=Type,x=num_units,fill=Type),show.legend=FALSE) +
    scale_fill_manual(values=c('#EFEFEF','#EFEFEF','#2D5D7B','#EFEFEF','#EFEFEF')) +
    labs(x='# of Units',y='',title='Some insight about Temporary STR') +
    theme_local()
```